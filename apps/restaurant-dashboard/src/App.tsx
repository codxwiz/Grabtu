...
function Dashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  async function saveRestaurantSettings(data: Partial<RestaurantSettings>) {
    return mutate(async () => {
      const updated = await api<RestaurantSettings>("/api/admin/settings", token, { method: "PATCH", body: JSON.stringify(data) });
      setSettings(updated);
      setUser(current => current ? { ...current, restaurant: { ...current.restaurant, name: updated.name } } : current);
    });
  }
